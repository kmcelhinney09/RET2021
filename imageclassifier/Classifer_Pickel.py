...
data_path = os.listdir("First5_numpy")
celeb_classifiers = {}
for data_dir in data_path:
    os.chdir('/home/teacheraccount/Documents/RET2021/imageclassifier')
    pair = os.path.join("First5_numpy",data_dir)
    #print(pair)
    os.chdir(pair)
    ...
    for image_file2 in file_2:
        image_file_data_path2 = os.path.join(image_dir[1], image_file2)
    ...
    celeb_classifiers[image_dir[0]] = clf
...
for data_dir in data_path:
    ...
    os.chdir(pair)
    image_dir = os.listdir()
    file_1 = os.listdir(image_dir[1])
    file_2 = os.listdir(image_dir[0])
    celeb_data = []
    celeb_labels = []

    for image_file in file_1:
        image_file_data_path3 = os.path.join(image_dir[1], image_file)
        print(image_file_data_path3)
        image_data = np.load(image_file_data_path3)
        celeb_data.append(image_data)
        celeb_labels.append(1)

    for image_file2 in file_2:
        image_file_data_path4 = os.path.join(image_dir[0], image_file2)
        print(image_file_data_path4)
        image_data = np.load(image_file_data_path4)
        celeb_data.append(image_data)
        celeb_labels.append(-1)
    ...
    clf = MLPClassifier(solver='sgd',
                        activation='relu',
                        alpha=1,
                        hidden_layer_sizes=(5756, 5756),
                        early_stopping=True,
                        # verbose=True
                        )

    print("{}------>{}".format(image_dir[1], image_dir[0]))
    clf.fit(x_train, y_train)
    ...
    # print the accuracy score of the model to the console
    #print(" The mean accuracy of this model is: {}".format(clf.score(x_test, y_test)))
    celeb_classifiers[image_dir[1]] = clf
